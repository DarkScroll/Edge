				...
				switch(href_list["field"])
					...
					if("mi_dis")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize_local(input("Please input minor disabilities list:", "Med. records", sanitize_local(src.active2.fields["mi_dis"], SANITIZE_TEMP), null)  as text, SANITIZE_BROWSER)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["mi_dis"] = t1
					if("mi_dis_d")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize_local(input("Please summarize minor dis.:", "Med. records", sanitize_local(src.active2.fields["mi_dis_d"], SANITIZE_TEMP), null)  as message, SANITIZE_BROWSER)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["mi_dis_d"] = t1
					if("ma_dis")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize_local(input("Please input major diabilities list:", "Med. records", sanitize_local(src.active2.fields["ma_dis"], SANITIZE_TEMP), null)  as text, SANITIZE_BROWSER)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["ma_dis"] = t1
					if("ma_dis_d")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize_local(input("Please summarize major dis.:", "Med. records", sanitize_local(src.active2.fields["ma_dis_d"], SANITIZE_TEMP), null)  as message, SANITIZE_BROWSER)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["ma_dis_d"] = t1
					if("alg")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize_local(input("Please state allergies:", "Med. records", sanitize_local(src.active2.fields["alg"], SANITIZE_TEMP), null)  as text, SANITIZE_BROWSER)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["alg"] = t1
					if("alg_d")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize_local(input("Please summarize allergies:", "Med. records", sanitize_local(src.active2.fields["alg_d"], SANITIZE_TEMP), null)  as message, SANITIZE_BROWSER)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["alg_d"] = t1
					if("cdi")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize_local(input("Please state diseases:", "Med. records", sanitize_local(src.active2.fields["cdi"], SANITIZE_TEMP), null)  as text, SANITIZE_BROWSER)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["cdi"] = t1
					if("cdi_d")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize_local(input("Please summarize diseases:", "Med. records", sanitize_local(src.active2.fields["cdi_d"], SANITIZE_TEMP), null)  as message, SANITIZE_BROWSER)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
					...
					if("vir_name")
						...
						if (v)
							var/t1 = sanitize_local(input("Please input pathogen name:", "VirusDB", sanitize_local(v.fields["name"], SANITIZE_TEMP), null)  as text, SANITIZE_BROWSER)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active1 != a1))
								return
							v.fields["name"] = t1
					if("vir_desc")
						var/datum/data/record/v = locate(href_list["edit_vir"])
						if (v)
							var/t1 = sanitize_local(input("Please input information about pathogen:", "VirusDB", sanitize_local(v.fields["description"], SANITIZE_TEMP), null)  as message, SANITIZE_BROWSER)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active1 != a1))
								return
