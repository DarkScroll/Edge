...
/datum/category_item/player_setup_item/general/flavor/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["flavor_text"])
		switch(href_list["flavor_text"])
			if("open")
			if("general")
				var/msg = sanitize(input(usr,"Give a general description of your character. This will be shown regardless of clothing, and may include OOC notes and preferences.","Flavor Text",lhtml_decode(pref.flavor_texts[href_list["flavor_text"]], SANITIZE_TEMP)) as message, extra = 0, mode = SANITIZE_BROWSER)
				if(CanUseTopic(user))
					pref.flavor_texts[href_list["flavor_text"]] = msg
			else
				var/msg = sanitize(input(usr,"Set the flavor text for your [href_list["flavor_text"]].","Flavor Text",lhtml_decode(pref.flavor_texts[href_list["flavor_text"]], SANITIZE_TEMP)) as message, extra = 0, mode = SANITIZE_BROWSER)
				if(CanUseTopic(user))
					pref.flavor_texts[href_list["flavor_text"]] = msg
		SetFlavorText(user)
		...
		switch(href_list["flavour_text_robot"])
			...
			if("Default")
				...
				var/msg = sanitize(input(usr,"Set the flavour text for your robot with [href_list["flavour_text_robot"]] module. If you leave this empty, default flavour text will be used for this module.","Flavour Text",lhtml_decode(pref.flavour_texts_robot[href_list["flavour_text_robot"]], SANITIZE_TEMP)) as message, extra = 0, mode = SANITIZE_BROWSER)
				if(CanUseTopic(user))
					pref.flavour_texts_robot[href_list["flavour_text_robot"]] = msg
