...
/datum/category_item/player_setup_item/general/background/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["nt_relation"])
		...
		var/new_medical = sanitize(input(user,"Enter medical information here.","Character Preference", lhtml_decode(pref.med_record, SANITIZE_TEMP)) as message|null, MAX_PAPER_MESSAGE_LEN, extra = 0, mode = SANITIZE_BROWSER)
		if(!isnull(new_medical) && !jobban_isbanned(user, "Records") && CanUseTopic(user))
			pref.med_record = new_medical
		return TOPIC_REFRESH

	else if(href_list["set_general_records"])
		var/new_general = sanitize(input(user,"Enter employment information here.","Character Preference", lhtml_decode(pref.gen_record, SANITIZE_TEMP)) as message|null, MAX_PAPER_MESSAGE_LEN, extra = 0, mode = SANITIZE_BROWSER)
		if(!isnull(new_general) && !jobban_isbanned(user, "Records") && CanUseTopic(user))
			pref.gen_record = new_general
		return TOPIC_REFRESH

	else if(href_list["set_security_records"])
		var/sec_medical = sanitize(input(user,"Enter security information here.","Character Preference", lhtml_decode(pref.sec_record, SANITIZE_TEMP)) as message|null, MAX_PAPER_MESSAGE_LEN, extra = 0, mode = SANITIZE_BROWSER)
