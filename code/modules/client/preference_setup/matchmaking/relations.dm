...
/datum/category_item/player_setup_item/relations/OnTopic(var/href,var/list/href_list, var/mob/user)
	...
	if(href_list["relation_info"])
		...
		var/info = sanitize_local(input("Character info", "What would you like the other party for this connection to know about your character?",sanitize_local(pref.relations_info[R], SANITIZE_TEMP)) as message|null, SANITIZE_BROWSER)
		if(info)
